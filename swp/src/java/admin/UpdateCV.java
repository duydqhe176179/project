...
public class UpdateCV extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        mdao.updateCV(idmentor, cv.getFullname(), cv.getAvatar(), cv.getPhone(),
                cv.getDob(), cv.getSex(), cv.getAddress(), cv.getProfession(),
                cv.getPro_introduc(), cv.getArchivement_descition(), cv.getFramework(), 
                cv.getExperience(), cv.getEducation(), cv.getMyservice(), cv.getCost());
